...
import { Search, MapPin, Clock, Building2, DollarSign, Filter } from 'lucide-react';
    ...
    const fetchJobs = async () => {
      try {
        const response = await axios.get(
          `http://localhost:${import.meta.env.VITE_PORT}/api/v1/dashboard/`,
          { withCredentials: true }
        );
        console.log(response.data.companies)

        setJobs(response.data.companies)
      } catch (error) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          {jobs.map((company : CompanyInterface) => (
            <button
              key={company.id}
            ...
            >
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{company.company_info.jobs.length} open positions</p>
                </div>
              </div>
            </button>
          ))}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {(selectedCompany ? selectedCompany.company_info.jobs : jobs.flatMap((company: CompanyInterface) => company.company_info.jobs)).map((job) => (
            <div
              key={job.id}
      ...
      {showApplicationModal && selectedJob && (
        <JobApplicationForm id={selectedJob.id}
          job={{
            title: selectedJob.title,
            company: selectedCompany
        ? selectedCompany.name
        : jobs.find((company: CompanyInterface) =>
            company.company_info.jobs.some(job => job.id === selectedJob.id)
          )?.name || ''
          }}
