...
const jobs = axios.get(`http://localhost:${import.meta.env.VITE_PORT}/api/v1/Companydashboard/`,{withCredentials: true});
...
export default function CompanyDashboard() {
  ...
  const [mockJobPostings, setMockJobPostings] = useState([]);

  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const response = await axios.get(
          `http://localhost:${import.meta.env.VITE_PORT}/api/v1/Companydashboard/`,
          { withCredentials: true }
        );
        const jobs = response.data.jobs.rows.map((job: any) => ({
          id: job.id,
          title: job.role_name,
          department: job.department,
          location: job.location,
          type: job.job_type,
          applicants: job.application_count || 0, // Default to 0 if null
          status: job.post_status,
          salary: `$${job.min_salary} - $${job.max_salary}`,
          posted: new Date(job.time).toLocaleDateString(), // Format time
        }));
        console.log("Transformed Job Data:", jobs);
        setMockJobPostings(jobs);
      } catch (error) {
