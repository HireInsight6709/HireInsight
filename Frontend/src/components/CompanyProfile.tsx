...
export default function CompanyProfile() {
  ...
  useEffect(()=>{
    async function Data() {
      ...
      const data = await axios.get(`http://localhost:${import.meta.env.VITE_PORT}/api/v1/Companyprofile/`, 
        { withCredentials : true })
        console.log(data.data.information);
        if(data.data.information != undefined){
          const response = data.data.information;
          
          let url : string = response[5];
          if(url.includes("No Logo URL Provided")){
            url = `https://images.unsplash.com/photo-1516387938699-a93567ec168e?ixlib=rb-1.2.1&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80`
          }

          const obj : CompanyData = {
            name : response[0],
            description : response[1],
            employeeCount :response[2],
            industry : response[3],
            location : response[4],
            logo : url,
            website : response[6]
          ...
          alert("An error occured while fetching details")
        }
    }
  Data();
  },[])
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    ...
    try{
      await axios.post(`http://localhost:${import.meta.env.VITE_PORT}/api/v1/Companyprofile/`, {companyData : companyData},
        { withCredentials : true })
      ...
      console.log(e)
      alert("Some internal server error hass occured")
    }
  };
